 ...
 */
public final class GLSimpleShape extends GLObject {
	 ...
	 */
	public GLSimpleShape(OldAndroidRenderer renderer, WWSimpleShape object, long worldTime) {
		...
		if (object instanceof WWBox || object instanceof WWCylinder) {
			if (object.getTwist() == 0.0 && object.getTaperX() == 0.0 && object.getTaperY() == 0.0) {
		...
		if (hasEnds && !(object.fixed && object.sideAttributes[WWObject.SIDE_BOTTOM].transparency == 1)) {
		...
		if (smoothSides) {
			...
			if (!(object.fixed && object.sideAttributes[WWObject.SIDE_SIDE1].transparency == 1)) {
			...
			for (int side = 0; side < nsides; side++) {
				...
				if (startVertex <= endVertex && !(object.fixed && object.sideAttributes[WWObject.SIDE_SIDE1 + side].transparency == 1)) {
		...
		if (hollow != 0.0) {
			...
			if (smoothSides) {
				...
				// For smooth surfaces there is only one side. This is so that normals will be properly
				// created for when the sides are smooth.
				if (!(object.fixed && object.sideAttributes[WWObject.SIDE_INSIDE1].transparency == 1)) {
				...
				for (int side = 0; side < nsides; side++) {
					int startVertex = Math.max(firstVertex, side * verticesPerSide);
					int endVertex = Math.min(lastVertex, (side + 1) * verticesPerSide - 1);
					if (startVertex <= endVertex && !(object.fixed && object.sideAttributes[WWObject.SIDE_INSIDE1 + side].transparency == 1)) {
		...
		if (cutStart != 0.0 || cutEnd != 1.0) {
			...
			if (!(object.fixed && object.sideAttributes[WWObject.SIDE_CUTOUT1].transparency == 1)) {
			...
			if (!(object.fixed && object.sideAttributes[WWObject.SIDE_CUTOUT2].transparency == 1)) {
		...
		if (hasEnds && !(object.fixed && object.sideAttributes[WWObject.SIDE_TOP].transparency == 1)) {
